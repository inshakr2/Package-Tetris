...
Package Tetris의 code-reviewer는 V3 Core 기능 증분이 기존 적재 계산, 저장, 백업, 3D 결과 확인 흐름을 깨지 않는지 검토한다. 검토는 미적 요소보다 버그, 회귀, 테스트 공백, 데이터 정합성, 성능 리스크를 우선한다.
...
- V3 구조 분해는 기능 변경으로 포장하지 않고, 실제 import 경계와 테스트가 문서와 일치하는지 확인한다.
- V2 검증 증거는 역사적 기준으로 두고, V3 증분은 별도 테스트와 브라우저 evidence로 현재 HEAD를 증명해야 한다.
