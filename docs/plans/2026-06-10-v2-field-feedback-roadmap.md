...
3. 현재 작업 물량 컬럼을 확정한다: 저장 박스 컬럼에 `수량`, `아래층우선`을 추가한다.
...
8. 현재 작업 import는 기존 저장 박스와 같은 이름이면 치수/깨짐주의가 같은지 검증한 뒤 재사용한다.
9. 저장 박스에 없는 현재 작업 import 행은 저장 박스 템플릿을 만들고 바로 현재 작업에 추가한다.
...
- 현재 작업 `.xlsx` import는 수량과 `아래층우선` 값을 함께 반영한다.
- 저장 박스 등록 샘플과 현재 작업 샘플을 각각 내려받을 수 있다.

**2026-06-11 implementation note:**
- 저장 박스 일괄등록과 별도로 현재 작업 물량 `.xlsx` import를 추가했다. 현재 작업 샘플은 `상위그룹`, `하위그룹`, `박스명`, `가로mm`, `세로mm`, `높이mm`, `무게kg`, `깨짐주의`, `수량`, `아래층우선` 순서를 사용한다.
- 현재 작업 import는 파일 선택 즉시 반영하지 않고 `현재 작업 엑셀 미리보기`에서 `추가할 박스`와 `오류 행`을 확인한 뒤 `현재 작업에 추가`로 적용한다.
- 기존 저장 박스와 같은 이름이면 치수/깨짐주의가 같은지 검증해 재사용하고, 없는 박스는 저장 박스로 만든 뒤 현재 작업에 추가한다. 따라서 업무 자동화 파일이 매일 출고 물량을 만들어도 저장 박스 라이브러리와 현재 작업 수량을 같은 흐름에서 관리할 수 있다.
- 현재 작업 카드의 `총 부피` 타일이 1280px 현장 PC 폭에서 깨지지 않도록 전용 칼럼과 최소 폭을 적용했다.
- 현장 UI 피드백에 따라 현재 작업 import에도 `현재 작업 포맷 보기` dialog를 추가했다. 저장 박스 import와 동일하게 열 설명, 샘플 행, 샘플 다운로드, 파일 선택 CTA를 제공한다.
...
- 추가 박스별 사용자 지정 우선순위를 연결했다. `기본` / `먼저 추가` / `최우선 추가`로 지정하면 추천 결과와 별도로 `지정 우선 결과` variant가 만들어지고, 같은 우선순위 안에서는 박스명 순서로 계산된다.
- 기준 결과 변경 또는 선택 초기화 시 추가 박스 선택, 지정 수량, 우선순위 조건을 함께 비우고 화면 상태 문구로 초기화 이유를 알린다.
- 1280px 현장 PC 폭에서 현재 작업 카드와 추가 박스별 조건 행이 과도하게 압축되지 않도록 1360px 이하 안전 브레이크포인트를 추가했다.
...
### 9.3 Partial Support Area Calculation
...
### 9.4 Additional Simulation Optimization
