...
- UI 문구는 숫자 우선순위 대신 현장 작업자가 바로 판단할 수 있는 3단계 버튼으로 제공한다. 내부 값은 0/5/10으로 정규화되어 기존 엔진의 `loadPriority` 정렬과 결과 최신성 fingerprint에 반영된다.
- 실행 전 확인 카드에는 하단 우선으로 지정한 현재 작업 항목 수를 요약한다. 작업자가 결과 만들기 직전에 우선 적재 설정 여부를 다시 확인할 수 있게 하기 위함이다.
- 회귀 테스트는 작업 항목 단위 저장, 저장 템플릿 비변경, 실행 전 확인 요약, 모바일 48px 터치 타깃을 확인한다.
...
- 추가 박스별 사용자 지정 우선순위를 연결했다. `기본` / `먼저 추가` / `가장 먼저`로 지정하면 추천 결과와 별도로 `지정 우선 결과` variant가 만들어지고, 사용자가 정한 추가 순서 기준의 결과를 바로 비교할 수 있다.
- 남은 Phase 7 작업은 브라우저 화면 검증 기반의 미세 조정이다.
