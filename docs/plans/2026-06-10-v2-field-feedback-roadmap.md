...
5. 허용되지 않는 파일 확장자, 빈 workbook, 빈 sheet, 알 수 없는 컬럼, prototype pollution key 방어 테스트를 작성한다.
6. 업로드 후 즉시 저장하지 않고 `미리보기 -> 오류 확인 -> 가져오기` 흐름으로 구현한다.
7. 가져오기 결과에 성공 건수, 실패 행, 중복 처리 결과를 표시한다.
8. 라이브러리 bundle 영향과 static export build 통과 여부를 확인한다.
...
- 작업별 하단 우선 설정 UI를 현재 작업 박스 카드에 연결했다. 저장된 박스 템플릿은 그대로 두고 `DraftBlockItem.loadPriority`만 갱신하므로, 같은 박스도 출고 작업마다 `기본` / `먼저 바닥에` / `가장 먼저`를 다르게 지정할 수 있다.
- UI 문구는 숫자 우선순위 대신 현장 작업자가 바로 판단할 수 있는 3단계 버튼으로 제공한다. 내부 값은 0/5/10으로 정규화되어 기존 엔진의 `loadPriority` 정렬과 결과 최신성 fingerprint에 반영된다. V1/외부 백업에서 숫자 단계가 다르게 들어오면 5 미만은 기본, 5 이상 10 미만은 `먼저 바닥에`, 10 이상은 `가장 먼저`로 보정한다.
...
- 추가 박스별 사용자 지정 우선순위를 연결했다. `기본` / `먼저 추가` / `가장 먼저`로 지정하면 추천 결과와 별도로 `지정 우선 결과` variant가 만들어지고, 같은 우선순위 안에서는 박스명 순서로 계산된다.
