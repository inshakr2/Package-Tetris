...
- 현재 작업 import는 `박스명 / 작업수량 / 적재위치타입` 3개 컬럼만 받는다. 박스명은 저장된 박스 라이브러리에 반드시 있어야 하며, 없는 박스명은 새 저장 박스로 만들지 않고 오류 행으로 안내한다. `적재위치타입`은 1=기본, 2=아래우선 숫자만 허용한다. 기존 `아래층우선타입` 컬럼은 읽기 호환만 유지한다.
