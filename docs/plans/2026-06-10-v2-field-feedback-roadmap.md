...
**Architecture:** `main`은 검증된 현장 시연 기준으로 유지하고, `v2`는 다음 현장 피드백 개발 기준으로 운영한다. V2는 프론트 단독 구조를 유지하되, 박스 라이브러리 스키마, 적재 정책, 결과 3D 표현, 추가박스 시뮬레이션을 단계적으로 확장한다. 모든 계산 정책 변경은 적재 엔진, 결과 검증, 추가 시뮬레이션에서 같은 정책 객체를 공유해야 한다.
...
- `main`은 검증된 현장 시연 기준이다.
- V1 완료 범위와 운영 전 파일럿 판단은 `docs/v1-readiness.md`에 역사 기준으로 보관한다.
- 현장 피드백 기반의 추가 기능 개발은 `v2`에서 진행한다.
- V1에서 아직 처리하지 못한 후보 기능도 모두 V2 backlog로 이동한다.
- 시연 기준에 치명적인 실행 불가, 데이터 손실, 빌드 실패, 보안성 문제가 발생하면 hotfix 후 `main -> v2`로 반영한다.

### 1.2 V2 브랜치 운영

- `v2`는 다음 현장 피드백 개발 기준이다.
- `main`은 검증된 현장 시연 기준으로 유지한다.
- 통합 검증을 통과한 변경만 `main`으로 병합한다.
- 각 기능 증분은 가능한 작은 커밋으로 나누고, 검증이 끝난 변경만 원격 `v2`에 push한다.
- V2 기능 개발 중 `main` hotfix가 발생하면 `main -> v2`로 먼저 반영해 테스트 기준을 맞춘다.
...
3. 현재 작업 물량 컬럼을 확정한다: `박스명`, `작업수량`, `적재위치타입` 3개 컬럼만 받는다.
...
8. 현재 작업 import는 저장된 박스명을 기준으로 앞뒤 공백과 대소문자는 보정해 현재 작업에 추가한다. 단, 글자가 다른 박스명은 오류 행으로 안내한다.
...
- 현재 작업 `.xlsx` import는 `작업수량`과 숫자형 `적재위치타입` 값을 함께 반영한다.
- 저장 박스 등록 샘플과 현재 작업 샘플을 각각 내려받을 수 있다.

**2026-06-11 implementation note:**
- 저장 박스 일괄등록과 별도로 현재 작업 물량 `.xlsx` import를 추가했다. 현재 작업 샘플은 `박스명`, `작업수량`, `적재위치타입` 순서를 사용한다.
- 현재 작업 import는 파일 선택 즉시 반영하지 않고 `현재 작업 엑셀 미리보기`에서 `추가할 박스`와 `오류 행`을 확인한 뒤 `현재 작업에 추가`로 적용한다.
- 현재 작업 import는 `박스명 / 작업수량 / 적재위치타입` 3개 컬럼만 받는다. 박스명은 저장된 박스 라이브러리에 반드시 있어야 하며, 앞뒤 공백과 대소문자는 보정한다. 글자가 다른 박스명과 없는 박스명은 새 저장 박스로 만들지 않고 오류 행으로 안내한다. `적재위치타입`은 1=기본, 2=아래우선 숫자만 허용한다. 기존 `아래층우선타입` 컬럼은 읽기 호환만 유지한다.
- 저장 박스 import와 현재 작업 import는 컬럼명 기준으로 값을 읽는다. 열 순서가 바뀌어도 허용하지만, 알 수 없는 컬럼, 중복 컬럼, 누락 컬럼은 오류로 거부한다.
...
**2026-06-11 implementation note:**
- 작업별 배치 우선 설정 UI를 현재 작업 박스 카드에 연결했다. 저장된 박스 템플릿은 그대로 두고 `DraftBlockItem.loadPriority`만 갱신하므로, 같은 박스도 출고 작업마다 `기본` / `아래 우선`을 다르게 지정할 수 있다.
- UI 문구는 중복 의미를 제거한 2단계 버튼으로 제공한다. 내부 값은 0/5로 정규화되어 기존 엔진의 `loadPriority` 정렬과 결과 최신성 fingerprint에 반영된다. V1/외부 백업에서 10점 단계가 들어오면 `아래 우선`으로 보정한다.
- 실행 전 확인 카드에는 배치 우선으로 지정한 현재 작업 항목 수와 박스별 수량/단계를 요약한다. 작업자가 결과 만들기 직전에 우선 적재 설정 여부를 다시 확인할 수 있게 하기 위함이다.
...
- 순서 변경 인지성을 보강했다. 드래그 또는 위/아래 버튼으로 순서를 바꾸면 이동한 카드에 짧은 테두리/배지 강조와 `n순위로 이동됨` 문구를 표시하고, 완료 피드백은 자동으로 사라진다.
- 추가 박스 선택 해제를 보강했다. 선택 카드의 `선택 해제`는 해당 박스와 지정 수량 조건을 함께 제거하고, 남은 카드는 1순위부터 다시 정렬한다. 선택, 해제, 순서 변경 뒤에는 기존 미리보기를 그대로 쓰지 않고 다시 계산하도록 안내한다.
...
- 현장 피드백으로 접수된 기본 파레트 혼합 적재 케이스를 추가 시뮬레이션 회귀 테스트와 `field:audit` 기능 검증에 추가했다. 기준 결과는 기존 좌표를 잠그고, `1000 x 800 x 400mm` 박스를 최대 6개까지 1순위로 시도한 뒤 `965 x 300 x 200mm`, `600 x 250 x 150mm` 박스를 선택 순서대로 최대 계산한다. 검증 기준은 현재 결과보다 남은 부피가 나빠지지 않고, 모든 추가 variant가 경계/충돌/지지면 검증을 통과하는 것이다.
- 추가 시뮬레이션 성능을 보강했다. 추천 결과 계산에 필요한 선택 박스 순열 결과를 한 번만 계산하고, `선택 순서 결과`와 각 박스 우선 variant는 같은 순열 결과를 복제해 재사용한다. 기존 배치 좌표와 추가 결과의 안전 검증 기준은 유지하며, variant 간 `spaces` 배열은 공유하지 않는다.
- 부피는 남아 있어도 안전하게 받칠 바닥면이 부족하거나 빈 공간이 나뉘면 추가 가능 0개 안내로 보여준다. 이 안내는 기존 결과 안전 검증 실패와 구분해 계산 실패로 처리하지 않는다.

## 6. Role Reports And Product Manager Finalization

### 6.1 Business Analyst Report

- `main`과 `v2`의 분리는 타당하다. 검증된 현장 시연 기준에 대규모 변경을 바로 넣으면 테스트 기준이 흔들린다.
...
- `npm run v2:verify`를 fresh run으로 통과시킨다. 이 명령은 `npx next typegen`, `npm test`, `npx tsc --noEmit`, `npm run field:audit`, `npm run build`, `git diff --check`를 순서대로 실행한다.
