...
- 추가 박스 시뮬레이션의 최대 3개 선택, 선택 해제, 지정 수량 조건 정리, 선택 순서 기반 우선순위, 순위 이동 피드백, 다시 계산 안내, 추천/우선 결과 비교
...
아래 항목은 V2 문서에서 반복적으로 언급될 수 있지만, 현재 구현 범위와 후속 판단을 분리해서 관리한다. 구현되지 않은 기능은 문서에서 완료처럼 표시하지 않는다.

- `위로 배치 선호`: 현재 구현 범위가 아니다. 별도 승인 후 엔진 정책, 작업본 migration, 엑셀 import enum, 실행 전 확인 요약, 현장 가이드가 같은 cycle에서 준비될 때만 사용자에게 노출한다.
- 무게 기반 계산: 현재 `무게kg`는 검색, 엑셀, 백업용 metadata이며 현재 적재 계산에는 반영하지 않는다. 적재 계산에 반영하려면 무게 중심, 파레트 총중량, 층별 하중, 박스별 하중 한계와 검증 fixture를 먼저 정의한다.
- 여러 기기 자동 동기화: 현재는 IndexedDB와 JSON 백업 기준이다. 서버 저장, 계정, 권한, 충돌 해결 정책이 승인되기 전까지 V2 프론트 단독 범위에 넣지 않는다.
- 검증 증거와 현재 HEAD: `docs/verification/` 문서는 특정 커밋 또는 실행 시점의 증거다. 현재 HEAD에서 UI, 엔진, 저장, import 동작이 바뀌면 `npm run v2:verify`와 필요한 브라우저 검증을 다시 수행하고, 새 증거 문서를 남긴 뒤 완료로 말한다.
- 바람개비 signature 변경: `690 x 370 x 580mm` 8개 케이스는 1공간, 미적재 0개, invariant 통과가 제품 완료 조건이다. 대표 바람개비 signature가 달라지는 엔진 변경은 product-manager와 code-reviewer 확인 후 회귀 기준을 갱신한다.
- 현장 작업자에게 보이는 UI, 버튼, 모달, 화면 구조 변경은 product-manager 단독으로 결정하지 않는다. business-analyst, ui-designer, ui-ux-tester 검토를 거친 뒤 nextjs-developer가 구현한다.
- 완료로 표시하는 항목은 자동 검증 또는 브라우저 증거가 있어야 한다. UI 변경은 360px, 390px, 768px, 1280px 실측 또는 동등한 브라우저 검증 기록을 남기고, 엔진 변경은 `field:audit`와 회귀 테스트에 포함한다.
...
저장 박스 import는 `상위그룹`, `하위그룹`, `박스명`, `가로mm`, `세로mm`, `높이mm`, `무게kg`, `깨짐주의` 컬럼을 사용한다. 현재 작업 import는 저장된 박스명을 기준으로 `박스명`, `작업수량`, `적재위치타입`만 받는다. 현재 작업 import의 박스명은 앞뒤 공백과 대소문자를 보정하지만, 글자가 다른 박스명은 오류 행으로 안내한다. `적재위치타입`은 1=기본, 2=아래우선을 사용하며, 기존 파일 호환을 위해 `아래층우선타입` 컬럼은 읽기만 지원한다. 두 import 모두 컬럼명 기준으로 값을 읽어 열 순서가 바뀌어도 동작하며, 파일 선택 즉시 반영하지 않고 미리보기와 오류 행 확인 후 적용한다. 현재 작업 import의 중복 행은 오류가 아니라 합산 미리보기로 처리하고, 같은 박스명의 적재위치타입이 다르면 기존 설정 유지 경고를 보여준다.
...
- 추가 박스 시뮬레이션: 저장 박스 최대 3개 선택, 선택 해제, 선택 해제 시 지정 수량 조건 정리, 선택 순서 기반 우선순위, 수량 지정, 순위 변경 시 `n순위로 이동됨` 피드백, 조건 변경 후 다시 계산 안내, 결과 비교와 반영/취소
