...
describe("placement-detail-table-layout", () => {
  it("결과 뷰어 근처에서 배치 상세 모달을 열 수 있다", () => {
    ...
    const inspectionActionsIndex = source.indexOf('className="result-inspection-actions"');
    const placementButtonIndex = source.indexOf('openResultInspectionDialog("placement"', inspectionActionsIndex);
    const stackingButtonIndex = source.indexOf('openResultInspectionDialog("stacking"', inspectionActionsIndex);

    // Then
    assert.equal(hasPlacementPanel, true);
    assert.equal(
      inspectionActionsIndex > -1 && placementButtonIndex > inspectionActionsIndex && stackingButtonIndex > placementButtonIndex,
      true
    );
