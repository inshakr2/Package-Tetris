...
describe("v2 verification report document", () => {
  it("V2 현장 패치 검증 리포트는 자동 검증과 대표 현장 케이스를 추적 가능하게 남긴다", () => {
    ...
    assert.deepEqual(
      getDisallowedPathsAfterVerifiedCommit(metadata.verifiedImplementationCommit).filter(
        (path) => !isV3CorePath(path)
      ),
      []
    );
...
function isV3CorePath(path: string) {
  return (
    ...
    path === "src/components/workspace/delete-confirm-dialog.tsx" ||
    path === "src/components/workspace/draft-undo-toast.tsx" ||
    path === "src/components/workspace/number-field-input.tsx" ||
    path === "src/components/workspace/reset-current-work-dialog.tsx" ||
    ...
    path === "src/lib/workspace/draft-block-priority-layout.test.ts" ||
    path === "src/lib/workspace/draft-block-xlsx-import-layout.test.ts" ||
    path === "src/lib/workspace/field-number-input-layout.test.ts" ||
