...
describe("loading-instruction-file", () => {
  ...
  it("공간명이 있으면 한글 공간명과 선택 공간 번호를 파일명에 함께 넣는다", () => {
    ...
    // Then
    assert.equal(filename, "my-tetris-2.5톤반-냉장칸-space-1-loading-2026-06-09.txt");
  });
  ...
  it("공간명의 파일명 예약 문자는 하이픈으로 정리한다", () => {
    ...
    // Then
    assert.equal(filename, "my-tetris-트럭-냉장칸-A-상차-space-3-loading-2026-06-09.txt");
  });
  ...
  it("긴 공간명은 다운로드 폴더에서 다루기 쉬운 길이로 줄인다", () => {
    ...
    // Then
    assert.equal(filename, `my-tetris-${"a".repeat(48)}-space-1-loading-2026-06-09.txt`);
  });
