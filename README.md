...
- 현재 개발 흐름: `main`은 검증된 현장 시연 기준, `v2`는 다음 현장 피드백 개발 브랜치
...
- 저장된 박스 라이브러리, 상위/하위 그룹, 저장 박스/현재 작업 `.xlsx` 일괄등록
...
`main`은 검증된 현장 시연 기준입니다. `v2`는 다음 현장 피드백 개발 브랜치이며, 통합 검증을 통과한 변경만 `main`으로 병합하는 흐름을 사용합니다.
